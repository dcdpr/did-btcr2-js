import { getNetwork, StaticFeeEstimator } from '@did-btcr2/bitcoin';
...
import { AggregationRunner } from '../src/index.js';
  ...
  it('drives a single-participant SMT cohort to a verifiable MuSig2 signature', async function() {
    ...
    const result = await AggregationRunner.solo({
      ...
      onProvideTxData : async ({ beaconAddress, signalBytes, feeEstimator: provided }) => {
        ...
        // The lone member binds its nonce approval to the validated signal: anchor
        // it in an OP_RETURN. capturedSighash is computed after, over the final tx.
