...
- **CAS has a sensible default.** If no `cas` config is passed, `api.cas` defaults to a read-only HTTP gateway against the public IPFS Trustless Gateway `https://trustless-gateway.link`. A browser can read from it: it sends CORS headers. Configure `cas.rpcUrl`, `cas.blockstore`, or a custom `cas.executor` for write capability; `api.cas.writable` reports whether the configured backend accepts publishes (executors declare it via `CasExecutor.canPublish`; undefined means writable). `api.cas.probe()` checks that the backend answers a read: it reads a fixed identity block (no network retrieval, no write) through the optional `CasExecutor.probe`, and aborts at the CAS timeout. `btcr2 config doctor` uses it.
