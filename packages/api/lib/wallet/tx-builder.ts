...
import { opReturnScript } from '@did-btcr2/method';
import { hex } from '@scure/base';
import { Address, p2pkh, p2tr, p2wpkh, Transaction } from '@scure/btc-signer';

import { explorerTxUrl } from '../../src/index.js';
...
/** The block-explorer URL of a transaction, from the api presets (ADR 082). Regtest has none. */
export function explorerHint(network: Network, txid: string): string {
  return explorerTxUrl(network, txid) ?? '(no public explorer)';
}
...
/** The address of a public key for a script kind. */
export function addressForKind(publicKey: Uint8Array, kind: AddrType, network: Network): string {
  const net = getNetwork(network);
  switch (kind) {
    case 'p2pkh':  return p2pkh(publicKey, net).address!;
    case 'p2wpkh': return p2wpkh(publicKey, net).address!;
    case 'p2tr':   return p2tr(publicKey.slice(1), undefined, net).address!;
  }
}

/** Add one input of the given script kind. A P2PKH input needs the previous transaction hex. */
function addInput(
  tx: Transaction,
  utxo: Utxo,
  kind: AddrType,
  ...
  const base = { txid: utxo.txid, index: utxo.vout };
  switch (kind) {
    case 'p2pkh':
      if (!prevTxHex) throw new Error(`P2PKH input ${utxo.txid}:${utxo.vout} needs the previous transaction hex`);
      tx.addInput({ ...base, nonWitnessUtxo: hex.decode(prevTxHex) });
      break;
    case 'p2wpkh':
      tx.addInput({ ...base, witnessUtxo: { amount, script: p2wpkh(publicKey, net).script } });
      break;
    case 'p2tr': {
      const xOnly = publicKey.slice(1);
      tx.addInput({ ...base, witnessUtxo: { amount, script: p2tr(xOnly, undefined, net).script }, tapInternalKey: xOnly });
      break;
    }
  }
}
...
function buildTx(args: BuildArgs): Transaction {
  ...
  for (const inp of args.inputs) {
    addInput(tx, inp.utxo, inp.kind, args.sourcePublicKey, args.network, inp.prevTxHex);
  }
...
/**
 * Anchor a 32-byte signal as an OP_RETURN at the address of `secretHex` for
 * the script kind (default P2WPKH). Spends the largest UTXO of the address;
 * the change returns to the same address (so anchors at one address chain);
 * the OP_RETURN is the last output (the resolver reads the last vout).
 * Two-pass fee.
 */
export async function anchorSignal(args: {
  ...
  const secret = kp.secretKey.bytes;
  const pubkey = kp.publicKey.compressed;
  const address = addressForKind(pubkey, kind, args.network);
  ...
  const build = (feeSats: bigint): Transaction => {
    ...
    addInput(tx, utxo, kind, pubkey, args.network, prevTxHex);
