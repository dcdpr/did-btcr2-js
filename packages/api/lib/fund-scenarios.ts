 ...
 * addresses of the solo scenarios (one per anchor address, read from each
 * `funding.json`) and the shared address of every cohort.
 *
...
import {
  indexScenarioDirs, loadCohorts, parseNetworkArg, readJSON,
  type AddrType, type FundingFile,
...
function collectTargets(): Target[] {
  ...
  for (const [, dir] of indexScenarioDirs(network)) {
    ...
    const f = readJSON<FundingFile>(path);
