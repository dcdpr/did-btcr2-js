...
describe('Create Deterministic', () => {
  ...
  const networkDidEntries = Object.entries(dids);
  const idType = 'KEY';
  ...
  it('should create a deterministic key identifier and DID document from a publicKey',
    async () => {
      const did = await DidBtcr2.create(genesisBytes, { idType });
      expect(did).to.equal(dids.get('bitcoin'));
    });

  it('should create a deterministic key identifier and DID document from a publicKey and version',
    async () => {
      const did = await DidBtcr2.create(genesisBytes, { idType, version });
