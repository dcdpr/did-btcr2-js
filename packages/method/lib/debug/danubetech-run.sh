...
#
# By default this runs whatever `btcr2` is on PATH, which is the PUBLISHED
# binary. To exercise a local build (the usual reason to run this), point
# BTCR2_BIN at the built CLI entrypoint:
#
#   BTCR2_BIN="node $(git rev-parse --show-toplevel)/packages/cli/dist/esm/bin/btcr2.js" ./danubetech-run.sh
#
# Refresh danubetech-vectors.json from a driver checkout with ./danubetech-sync.sh.
#
# Requires: jq, GNU timeout, and a btcr2 CLI (PATH or $BTCR2_BIN)
#
set -uo pipefail
...
# The CLI under test. $BTCR2_BIN may carry arguments ("node /path/btcr2.js"),
# so it is split on whitespace into the command array.
read -r -a BTCR2_CMD <<<"${BTCR2_BIN:-btcr2}"
if ! command -v "${BTCR2_CMD[0]}" >/dev/null; then
  echo "btcr2 CLI not found: ${BTCR2_CMD[0]} (set \$BTCR2_BIN to a local build)"; exit 2
fi
...
btcr2_version="$("${BTCR2_CMD[@]}" --version 2>&1 | head -n1 || echo unknown)"
...
for example in "${EXAMPLES[@]}"; do
  ...
  declare -a cmd=("${BTCR2_CMD[@]}" resolve -i "${did}")
  options_file=""
