...
| `-m, --verification-method-id <id>` | A verification method ID present in the document's `capabilityInvocation` (validated before the update is constructed). Absolute (`did:btcr2:...#initialKey`) and relative (`#initialKey`) DID URLs are both accepted: the id is resolved against the source document before matching. | (required) | Verification method used to sign the deactivation. Its key must be resolvable in the keystore. |
| `-b, --beacon-id <json>` | JSON string (note: the value itself must be valid JSON, so shell-quote an extra pair of double quotes, e.g. `'"did:btcr2:...#initialP2WPKH"'`). Must match the `id` of a beacon service in the source document, in either the absolute or the relative spelling. | (required) | The beacon service whose Bitcoin address broadcasts the deactivation signal. |
