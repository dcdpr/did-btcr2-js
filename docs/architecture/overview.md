...
- **Transport adapters** (Nostr via relays; HTTP/REST via fetch + SSE; DIDComm stub) handle on-the-wire message delivery. Transports are interchangeable at construction time via `TransportFactory.establish({ type, role?, ... })`.
