---
title: "ADR 021: tsconfig Normalization and CJS via tsup"
---
...
- 8 of 9 src configs had no explicit `types` array to every `@types/*` package leaked into every build
- No explicit `lib` array to DOM globals were silently available in node-only packages (`bitcoin`, `kms`, `cli`)
...
- **Keep using `typedoc-vitepress-theme` for docs.** Considered. Rejected because the docs site was being rebuilt anyway as a contributor-only resource (see ADR 016, forthcoming) and the simpler TypeDoc + `projectDocuments` approach is a better fit.
